...
import { VertSpriteAssembler } from "db://assets/VertSprite/VertSpriteAssembler";
...
export class VertSprite extends UIRenderer {
    ...
    public changeMaterialForDefine() {
        ...
        if (texture instanceof cclegacy.TextureBase) {
            ...
            value =
                ...
                format ===
                    cclegacy.TextureBase.PixelFormat.RGB_A_PVRTC_4BPPV1 ||
        ...
        if (value && this.grayscale) {
            ...
            this._instanceMaterialType =
                InstanceMaterialType.USE_ALPHA_SEPARATED;
        } else if (this.grayscale) {
            this._instanceMaterialType = InstanceMaterialType.GRAYSCALE;
        } else {
            this._instanceMaterialType =
                InstanceMaterialType.ADD_COLOR_AND_TEXTURE;
        }
    ...
    protected _updateBuiltinMaterial() {
        ...
        if (
            this.spriteFrame &&
            this.spriteFrame.texture instanceof RenderTexture
        ) {
    ...
    private _resized() {
        ...
        if (this._spriteFrame) {
            ...
            if (
                expectedW !== actualSize.width ||
                expectedH !== actualSize.height
            ) {
    ...
    //     // 第0行（最下面一行，y=0）
    //     { x: 0, y: 0 }, // 顶点0: 左下角
    //     { x: this.width / 2, y: 0 }, // 顶点1: 下中
    //     { x: this.width, y: 0 }, // 顶点2: 右下角
    //
    //     // 第1行（中间一行，y=height/2）
    //     { x: 0, y: this.height / 2 }, // 顶点3: 中左
    //     { x: this.width / 2, y: this.height / 2 }, // 顶点4: 中心
    //     { x: this.width, y: this.height / 2 }, // 顶点5: 中右
    //
    //     // 第2行（最上面一行，y=height）
    //     { x: 0, y: this.height }, // 顶点6: 左上角
    //     { x: this.width / 2, y: this.height }, // 顶点7: 上中
    //     { x: this.width, y: this.height }, // 顶点8: 右上角
    // ];
}
